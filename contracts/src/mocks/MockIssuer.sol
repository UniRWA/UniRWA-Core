...
contract MockIssuer is Ownable {
    ...
    // Events
    event AssetRegistered(string indexed symbol, address indexed token, uint256 minimum);
    event PoolAuthorized(address indexed pool);
    event Purchase(address indexed buyer, string indexed symbol, uint256 usdcAmount, uint256 tokensReceived);
    ...
    function _executePurchase(
        ...
        (bool success, ) = address(rwaToken).call(
            abi.encodeWithSignature("mint(address,uint256)", recipient, tokensToMint)
        );
